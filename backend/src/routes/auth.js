...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    const license = await License.findOne({ where: { license_key: licenseKey, isUsed: false } });
    if (!license) {
...
router.post('/update-password', async (req, res) => {
  try {
    const { username, newPassword } = req.body; // Get username and new password from request body
